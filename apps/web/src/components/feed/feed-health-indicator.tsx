...
export function FeedHealthIndicator({
  ...
  return (
    <button
      ...
      title={errorMessage || t('feed.fetchFailCount', { count: errorCount })}
    >
    ...
    <div className="p-4 bg-[rgb(var(--bg-base))] border border-default rounded-lg">
        ...
        <div>
          ...
          {feed.checkedAt && (
            <p className="text-xs text-muted flex items-center gap-1 mt-1">
              ...
              {t('feedHealth.lastCheck')}: {formatDistanceToNow(new Date(feed.checkedAt), { addSuffix: true, locale: getLocale() === 'zh' ? zhCN : undefined })}
            </p>
          )}
        </div>
        <div className={cn(
          ...
          {feed.disabled ? t('feed.statusPaused') : feed.errorCount > 0 ? t('feed.statusFailed', { count: feed.errorCount }) : t('feed.statusNormal')}
        </div>
      </div>
      ...
      <div className="flex items-center gap-2">
        ...
        >
          ...
          {isRetrying ? t('feed.refreshing') : t('feed.retryBtn')}
        </button>
        <button
      ...
      {isOpen && (
        ...
        >
          <div className="flex items-center justify-between px-4 py-3 border-b border-default sticky top-0 bg-[rgb(var(--bg-elevated))]">
            <div>
              <h3 className="font-semibold text-sm">{t('feed.healthTitle')}</h3>
              <p className="text-xs text-muted">
                {t('feed.healthSummary', { errors: errorCount, paused: disabledCount })}
              </p>
            </div>
            <button
              onClick={onClose}
